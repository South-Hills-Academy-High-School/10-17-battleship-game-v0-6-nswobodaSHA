namespace SpriteKind {
    ...
    export const boat3 = SpriteKind.create()
    export const boat4 = SpriteKind.create()
...
function moveBoat (boatArray: any[]) {
    ...
    for (let previousBoatSprite of boatArray) {
        if (boatRotateArray[currentBoat] == "up") {
            grid.place(previousBoatSprite, tiles.getTileLocation(grid.spriteCol(cursor), grid.spriteRow(cursor) + iterator))
        } else {
            grid.place(previousBoatSprite, tiles.getTileLocation(grid.spriteCol(cursor) + iterator, grid.spriteRow(cursor)))
        }
        iterator += 1
    }
}
